...
class DragonScaler:
    ...
    def fit(cls, dataset: Dataset, continuous_feature_indices: list[int], batch_size: int = 64, verbose: int = 3) -> 'DragonScaler':
        ...
        if verbose >= 2:
            _LOGGER.info(f"Scaler fitted on {n_total} samples for {num_continuous_features} columns.")
    ...
    def fit_tensor(cls, data: torch.Tensor, verbose: int = 3) -> 'DragonScaler':
        ...
        if verbose >= 2:
            _LOGGER.info(f"Scaler fitted on {data.shape[0]} samples for {num_features} columns.")
