...
def plot_numeric_overview_boxplot(
    ...
    if strategy == "scale":
        ...
        zero_var_cols = numeric_df.columns[max_vals == min_vals]
        
        if not zero_var_cols.empty:
            if handle_zero_variance == "drop":
                numeric_df = numeric_df.drop(columns=zero_var_cols)
                _LOGGER.warning(f"Dropped zero-variance columns during scaling: {list(zero_var_cols)}")
                if numeric_df.empty:
                    _LOGGER.warning("No columns left after dropping zero-variance features.")
                    return
                # Recalculate after drop
                max_vals = numeric_df.max()
                min_vals = numeric_df.min()
        
        # Using pandas native operations to achieve Min-Max scaling
        numeric_df = (numeric_df - min_vals) / (max_vals - min_vals)
        
        # Intercept the NaNs generated by division by zero and set to 0.0
        if handle_zero_variance == "constant" and not zero_var_cols.empty:
            numeric_df[zero_var_cols] = 0.0
            _LOGGER.warning(f"Set zero-variance columns to 0 during scaling: {list(zero_var_cols)}")
            
        x_label = "Value (Min-Max Scaled)"
    elif strategy == "log":
