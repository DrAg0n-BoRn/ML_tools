...
__all__ = [
    "_PytorchSequenceDataset",
    "_BaseSequenceDataset"
]
...
class _PytorchSequenceDataset(Dataset):
    ...
    def __init__(self, 
        ...
        self._class_map: dict[str, dict[str, int]] = dict()
    ...
    def __getitem__(self, index):
        # 2. Lazily cast to tensor only when the Dataloader fetches a batch.
...
class _BaseSequenceDataset(_BaseDatasetMaker, ABC):
    """
    Abstract base class for sequence datasets.
    Handles shared chronological splitting, scaling, and validation logic.
    """
    def __init__(self, 
        ...
        if prediction_mode not in MLTaskKeys.ALL_SEQUENCE_TASKS:
            _LOGGER.error(f"Unrecognized prediction mode: '{prediction_mode}'. Valid modes are: {MLTaskKeys.ALL_SEQUENCE_TASKS}")
            raise ValueError()
        ...
        # Map target names to their column indices
        self._target_indices = [self._feature_names.index(t) for t in self._target_names]
        
        # Determine target types based on schema
        self._target_types = {
        ...
        # Clamp to 0 to prevent negative slicing
        ...
        # dummy target for target scaler
        dummy_y = pandas.Series(0.0, index=X_train.index)
        
        # --- 3. Scale Features and derive a custom Target Scaler ---
    ...
    def _create_windowed_dataset(self, data: numpy.ndarray, verbose: int = 3) -> _PytorchSequenceDataset:
        """Abstract method to define logic for window striding and feature extraction."""
        pass
    
    @property
    def last_dataset_sequence(self) -> numpy.ndarray:
        """
        Returns the final unscaled window of the entire dataset (initial sequence in inference).
        """
    ...
    @classmethod
    def from_bundle(cls, filepath: Union[str, Path], verbose: bool = True) -> '_BaseSequenceDataset':
        """
        ...
        bundle = torch.load(target_filepath, weights_only=False, map_location='cpu')
        ...
        instance.classes = list() # not used in sequence datasets
