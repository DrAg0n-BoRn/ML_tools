...
class DragonSequenceTransformer(_ArchitectureBuilder):
    ...
    def __init__(self, 
        ...
        Args:
            ...
            embedding_dim (int): The number of expected features in the encoder inputs. 
            heads (int): The number of heads in the multi-head attention models. Should evenly divide `embedding_dim`.
        ...
        self.input_projection = nn.Linear(self.in_features, embedding_dim)
        ...
        for target in targets:
            if target in schema.categorical_feature_names:
                ...
                self.target_heads[target] = nn.Linear(embedding_dim, cardinality)
            else:
                self.target_heads[target] = nn.Linear(embedding_dim, 1)
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        alibi_mask = get_alibi_mask(seq_len=seq_len, 
                                    nhead = self._heads,
                                    device = x.device, 
                                    is_causal = self._is_causal)
        # PyTorch 3D mask shape requirement: (batch_size * heads, seq_len, seq_len)
    ...
    def extra_repr(self) -> str:
        ...
        return (
            ...
            f"heads={self.model_hparams['heads']}, "
