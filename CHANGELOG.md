...
- ML_trainer:
    - Fixed a bug in `DragonTrainer` where the checkpoint callback's save directory was not being set correctly, leading to potential issues with artifact organization. The save directory is now properly attached to the checkpoint callback during initialization.
...
- ML_datasetmaster:
    - Fixed import error for the revamped class `DragonDatasetSequence`.
...
- ML_models_diffusion:
    - `DragonAutoencoderV2`: Introduced a modernized autoencoder architecture featuring Cross-Feature Transformers for inter-feature dependencies, Variational Autoencoder (VAE) latent regularization, and pluggable Piecewise Linear Embeddings (PLE) for robust tabular anomaly handling.
    - `RMSNorm` and `SwiGLU` helper modules for modern normalization and gated feed-forward networks.
    - `DiTBlockFlashV2` introducing QK-Normalization, RMSNorm, and SwiGLU.
    - `_BaseDragonDiT` and `_BaseDragonDiTGuided` abstract base classes to unify model logic, time embeddings, and ODE solver loops.
    - `DragonDiTV2` and `DragonDiTGuidedV2` models leveraging the upgraded V2 transformer blocks.
    - `cfg_rescale` parameter to the conditioned `generate_sequence()` method to prevent feature variance collapse.
- ML_configuration:
    - `DragonAutoencoderV2Params`: Added a new configuration class to encapsulate hyperparameters for the new V2 autoencoder, including latent dimension size, number of transformer layers, and PLE settings.
    - `DragonDiTV2Params`, New configuration classes for the V2 DiT and DiT-Guided models.
- ML_inference_diffusion:
    - Updated all diffusion inference handlers to support the new V2 autoencoder, allowing for seamless generation and decoding of synthetic tabular data with enhanced feature representation and regularization.
- ML_trainer:
    - `DragonTabularDiTTrainer`: Updated the trainer to accept either the original V1 autoencoder or the new V2 autoencoder as the token embedder.
    - Optimal Transport Flow Matching (OT-CFM) support within `DragonTabularDiTTrainer._prepare_flow_matching_batch()` using the Hungarian algorithm for V2 models.

### Changed

- ML_models_diffusion:
    - `DragonAutoencoder`: Refactored the base autoencoder to support both V1 and V2 architectures, enabling dynamic loading of either version based on the model artifact.
- ML_trainer:
    - `DragonAutoencoderTrainer._train_step()`: Updated the training loop to conditionally unpack and handle the 3-tensor VAE outputs `(z, mu, logvar)` generated by the new V2 autoencoder during training.
    - `_compute_autoencoder_loss()`: Upgraded the loss helper function to optionally accept `mu` and `logvar` parameters, computing and applying a Beta-VAE KL-Divergence penalty alongside the existing homoscedastic uncertainty weighting.
    - `DragonTabularDiTTrainer`: Added dynamic architecture detection to handle both legacy V1 and new V2 model instances seamlessly.
    - `DragonTabularDiTTrainer`: Added the `use_ot_cfm` parameter to control the use of OT-CFM during training. Not compatible with V1 models that were not originally trained with OT-CFM, as it will break the learned distribution.
- ML_inference_diffusion:
    - `.plot()`: Method moved to the base class and renamed `base_plot_title` parameter to `plot_title` for clarity.
    - Removed the `save_dir` parameter from the constructor of all diffusion inference handlers. The `plot_metrics()` method now requires an explicit `save_dir` argument to specify where plots should be saved, enhancing flexibility and decoupling plot saving from object instantiation.
    - `DragonDiTGuidedGenerator.generate_plot_multi()`: Renamed to `generate_multi()`. Replaced the `targets` list parameter with `target_range` (start, end, step) and `batch_size` with `batch_per_step`. Removed plotting and saving parameters, updating the method to return a single consolidated DataFrame.
    - `DragonDiTGuidedGenerator.generate()`: Removed the `autosave` parameter. Added an optional `target_name` parameter to append the conditioning target as a new column to the output DataFrame.
    - `DragonDiTGenerator.generate()`: Removed the `autosave` parameter to strictly focus the method on data generation.
    - `_BaseDiffusionGenerator`, `DragonDiTGenerator`, and `DragonDiTGuidedGenerator`: Updated initialization type hints to fully support `DragonDiTV2` and `DragonDiTGuidedV2` models.
    - `DragonDiTGuidedGenerator.generate()` and `generate_multi()`: Exposed the `cfg_rescale` parameter to allow user control over V2 variance rescaling during inference.
- ML_callbacks:
...
- ML_inference:
    - Added `_BaseVisionInferenceHandler` to centralize duplicated vision-specific code (`set_transform`, `predict_from_pil`, `predict_from_file`, and `predict_from_directory`).
    - Implemented a static color palette generation method (`_initialize_color_map`) for vision handlers to lock distinct colors to specific labels globally.

### Changed

- ML_inference:
    - Refactored `DragonObjectDetectionInference` and `DragonSegmentationInference` to inherit from the newly created `_BaseVisionInferenceHandler`.
    - Updated `_create_overlapped_image` in both Object Detection and Segmentation handlers to utilize the initialized global color map, resolving the issue of inconsistent colors shifting across image tiles.
    - Overrode the `set_class_map` method in vision handlers to ensure the color palette is automatically regenerated if class labels are updated dynamically.
...
- ML_evaluation:
    - `autoencoder_metrics()`, fixed plot axis and label formatting to ensure proper display of scientific notation when required.
...
- ML_trainer:
    - `DragonTabularDiTTrainer`, move tokenizer to the correct device at initialization. Override `.to_cpu()` and `.to_device()` methods to ensure the tokenizer is moved along with the model, preventing device mismatch errors during training and inference.
...
- ML_models_diffusion:
    - Fixed a reconstruction bug in the method `from_artifact_finder()` for the models `DragonAutoencoder`, `DragonDiT`, `DragonDiTGuided`.
...
- Logging messages in new modules for better clarity and consistency.
...
- outlier_detection:
    - `drop_outliers_rule()`, added "reset_index" parameter to reset the index after dropping outliers.
    - `drop_outliers_mask()`, added "reset_index" parameter to reset the index after dropping outliers.

- ML_datasetmaster:
    - `DragonDataset`, added support for "diffusion" and "autoencoder" tasks.

- ML_models:
    - `DragonTabularTransformer`, Changed the forward pass to use average pooling over the sequence dimension instead of a \[CLS\] token, which allows the model to learn from all features equally and is more stable for tabular data.
    - `DragonAutoInt`, modified forward pass to include intra-block residual connections for improved training stability and performance.
    - `DragonTabNet`, modified forward pass to accumulate sparsity loss matching the original implementation by minimizing the entropy of the mask.
    - Optimized some advanced models for faster inference by removing unnecessary operations and improving tensor handling.

- schema:
    - `FeatureSchema`, added methods "to_dict()" and "from_dict()".

### Added

- keys:
    - `TaskKeys`, Added keys for "autoencoder" and "diffusion" tasks.

- New module: "ML_models_diffusion"
    - `DragonAutoencoder`, Autoencoder model for tabular data.
    - `DragonDiT`, Diffusion model using flow matching, modality-agnostic.
    - `DragonDiTGuided`, Guided version of the DiT model for tabular regression tasks.

- ML_trainer:
    - `DragonAutoencoderTrainer`, New trainer class for autoencoder used in tabular data.
    - `DragonTabularDiTTrainer`, New trainer class for training DiT models on tabular data.

- ML_configuration:
    - `FormatAutoencoderMetrics`, Format class for autoencoder training metrics.
    - `FormatTabularDiffusionMetrics`, Format class for tabular diffusion model training metrics.
    - `FinalizeAutoencoder`, Finalization handler for autoencoder models.
    - `FinalizeTabularDiffusion`, Finalization handler for tabular diffusion models.
    - `DragonAutoencoderParams`, Configuration parameters for the autoencoder model.
    - `DragonDiTParams`, Configuration parameters for the DiT model.

- ML_evaluation:
    - `autoencoder_metrics()`, Evaluates autoencoder performance using metrics like reconstruction error and latent space visualization.
    - `dit_generation_metrics()`, Evaluates the quality of samples generated by the DiT model using metrics like MMD and visualizations.

- ML_utilities:
    - `validate_torch_device()`, Validates the specified torch device string and returns a torch.device object.
